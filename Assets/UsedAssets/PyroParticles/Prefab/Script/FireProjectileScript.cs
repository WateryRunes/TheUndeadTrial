...
namespace DigitalRuby.PyroParticles
{
    ...
    public class FireProjectileScript : FireBaseScript, ICollisionHandler
    {
        ...
        public void HandleCollision(GameObject obj, Collision c)
        {
            ...
            {
                c.gameObject.GetComponent<PlayerController>().updateHealth(-30);
            }
            else if(c.gameObject.tag == "MeleeEnemy")
            {
                c.gameObject.GetComponent<MeleeEnemyController>().changeHealth(-30);
            }
            else if (c.gameObject.tag == "RangedEnemy")
            {
                c.gameObject.GetComponent<RangedEnemyController>().changeHealth(-30);
            }
        }
    }
}